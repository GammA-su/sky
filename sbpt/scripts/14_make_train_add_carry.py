"""Generate carry-trace addition dataset."""
...
from sbpt.data.synth_traces import iter_add_carry_trace
from sbpt.utils.runtime import configure_runtime, setup_logging


def main() -> None:
    logger = setup_logging("sbpt.make_add_carry")
    configure_runtime(logger=logger)
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    default_out = os.path.join(repo_root, "out", "train_add_carry.jsonl")
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        for row in iter_add_carry_trace(
            n=args.n,
            seed=args.seed,
